...
| route | press → carrier, cold | signals absorbed | warm over |
|---|---|---|---|
| `route=none` (no accessory) | 1.03–1.09 s | 3–5 | 23–30 ms |
| `route=BluetoothHFP` (Q2L) | 0.81 s | 1–2 | 23 ms |
...
Still untested: **macOS with the Q2L** — the SCO bring-up swapping the A2DP
device for the HFP one and posting `AVAudioEngineConfigurationChange`. Same
ordering, so it should be covered, but the accessory is paired to the phone and
`BU15FirstOverUITests` has to run on the Mac holding it. That is the test.
